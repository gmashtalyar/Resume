...
txt('**Основатель |**  [ФинТехДокументы](https://fintechdocs.ru)', '2023-н.в.')
st.markdown('''
Стартап корпоративных fintech-продуктов. Основной продукт - прилодение СУДЗ для управления кредитным риском (Django, Swift, Kotlin).
''')

txt('**Заместитель финансового директора |**  [Ариэль Металл](https://arielmetal.ru)', '2021-н.в.')
st.markdown('''
Металлотрейдер с выручкой `~25 млрд.р.` и оборотом +`500` т.тн.
- Постоение бизнес-плана компании с участием кросс-функциональных отделов (продажи, склад, логистика, финансы, персонал).
- Создал приложение для управления финансами `(web-forms & api)`. Автоматизированное планирование, выгрузка выписки, прогнозирование
ликвидности, отчетность, платежи.
- Планирование кратко- средне- долгосрочное.
...
* Риск менеджмент - операционного и кредитного риска по +`100` клиентам (МСБ) и +`50` дебиторам (РБК500).
  * Принял участие на кредитном комитете в +`50` структурных долговых сделках.
...
* Казначейские операции - управление финансовыми транзакциями компании с оборотами по счетам + `10 млрд.р`.
  ...
  * Участие в привлечении кредитного капитала компании.
